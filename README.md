## 👋 Hi, I’m Samson Tolulope Moradeyo 
[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering) <!--[![Website Badge](https://img.shields.io/badge/-github.com/psalmotee/-000000?style=for-the-badge&logo=Google-Chrome&logoColor=white&link=https://github.com/psalmotee/)](https://github.com/psalmotee/) --> [![Linkedin Badge](https://img.shields.io/badge/-samson-moradayo-211b26187-blue?style=for-the-badge&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/samson-moradayo-211b26187)](https://www.linkedin.com/in/samson-moradayo-211b26187) [![Twitter Badge](https://img.shields.io/badge/-@Psalmo_Tee-1ca0f1?style=for-the-badge&logo=twitter&logoColor=white&link=https://x.com/Psalmo_Tee)](https://x.com/Psalmo_Tee)

A passionate **Frontend Developer** with a knack for building responsive, user-friendly, and visually appealing web applications. Skilled in **HTML**, **CSS**, **JavaScript**, and **React**, and familiarize with **Vue**, and **TypeScript** with hands-on experience translating Figma designs into functional interfaces.

I’m deeply focused on **user experience**, **performance optimization**, and writing clean, maintainable code. I enjoy working on innovative products, solving UI challenges, and collaborating with diverse teams to push the boundaries of frontend development.

---
### 🚀 Tech Stack
- **Languages**: JavaScript (ES6+), HTML5, CSS3
- **Frameworks & Libraries**: React.js, Tailwind CSS, Ant Design, Chakra UI
- **Tools**: Git, GitHub, Firebase, Netlify, Vercel
- **Currently Learning**: Next.js, TypeScript, UI architecture best practices

--

## 🔍 Here's a quick summary about me:

- 😄 Pronouns: He/Him  
- 🔭 Currently growing as a Front-End Engineer  
- 💼 Job interests: Frontend Developer, React Developer, UI Engineer  
- 👯 Looking to contribute to the tech ecosystem and build with great teams  
- 💡 Open to open source, internships, and junior frontend roles  
- 🌱 Currently exploring: **Next.js**, **TypeScript**, and scalable component architecture  
- 🧠 Favorite tools: React, Tailwind CSS, Git, Firebase  

--

## 🌐 Connect with me:
- 📫 Email: samsonmoradeyo@gmail.com  
- 🐙 GitHub: [Psalmotee](https://github.com/psalmotee)  
- 💼 LinkedIn: [Samson Moradeyo](https://linkedin.com/in/samson-moradayo-211b26187)  
- 💻 Portfolio: [portfolio-psalmotee.netlify.app](https://portfolio-psalmotee.netlify.app)

--

⚡ Fun fact: I love turning UI designs into clean, functional code and learning from real-world challenges!
